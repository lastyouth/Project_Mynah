...
<%
    ...
    // 목록
    rset = appellation.getAppellationList(parmValue);

    /*============= 2. 페이징 관련 셋팅 ================*/
    totalCnt  = appellation.getAppellationTotalRow(parmValue);             // 총 게시물 갯수
...
<!--
    ...
    /* 읽기 페이지로 이동 */
    function goDisplay(appellation_id) {
        board.appellation_id.value = appellation_id; 
    ...
    function goDelete(){
        ...
        var isChk = false;
         ...
         if( confirm("선택하신 리스트를 삭제 하시겠습니까?")) {
             ...
             frm.action   = "appellation_write_proc.jsp";
             frm.submit();
         }
    }
...
%>				
			<tr>
				<td align="center"><a href="javascript:goDisplay('<%=appellation_id%>');" class="skin"><%=conference_name%></a></td>
				<td align="center">
					<a href="javascript:goDisplay('<%=appellation_id%>');" class="skin">
					<%=appellation_name%>
					</a>
				</td>
				<td align="center"><a href="javascript:goDisplay('<%=appellation_id%>');" class="skin"><%=reg_date%></a></td>
				<td align="center"><a href="javascript:goDisplay('<%=appellation_id%>');" class="skin"><%=worker_name%></a></td>
			</tr>
<%      } 
    }
    %> 
