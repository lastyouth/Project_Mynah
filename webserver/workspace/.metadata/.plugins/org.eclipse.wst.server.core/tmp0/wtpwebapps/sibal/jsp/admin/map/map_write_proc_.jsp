			...
			if(request.getContentLength() < fileupload.getSizeMax()) {
				...
				for(int i=0;i < size;i++) {
					...
					if(fileitem.isFormField()){												//파라미터일 경우 true
						...
						if (fileitem.getFieldName().equals("num"))num = fileitem.getString("UTF-8");	
						if (fileitem.getFieldName().equals("conference_id"))conference_id = fileitem.getString("UTF-8");	
						for (int x=0; x <intNum ; x++){
							if (fileitem.getFieldName().equals("title_"+x))parmValue.put("title_"+x    , fileitem.getString("UTF-8")    );
						}
						//if (fileitem.getFieldName().equals("files"))files = fileitem.getString("UTF-8");	
						...
						if (!(fileitem.getName()).equals("")) {
							if(fileitem.getSize() > 0) {
								...
								for (int x=0; x <intNum ; x++){
									if (fileitem.getFieldName().equals("file_"+x))parmValue.put("file_"+x    , tmpFileName    );
								}
    ...
    try {	   
        if( isOk ) {
        	if( "ins".equals(modeType)){
            	...
            	//map.bannerModify(parmValue);
            } else if( "del".equals(modeType)){  
            	//map.bannerDeleteContents(parmValue);  
            }
       }
