...
<%
    ...
    // 목록
    rset = banner.getBannerList(parmValue);

    /*============= 2. 페이징 관련 셋팅 ================*/
    totalCnt  = banner.getBannerTotalRow(parmValue);             // 총 게시물 갯수
...
<!--
    ...
    /* 읽기 페이지로 이동 */
    function goDisplay(banner_id) {
        board.banner_id.value = banner_id; 
        board.b_mode.value       = "W";
       // board.target             = "_self";
        board.action             = "<%= request.getRequestURI() %>";
        board.submit();
...
<%    
        ...
        while( rset.next()){
        	...
        	banner_conference_name     = StringUtil.defaultIfEmpty(rset.getString("conference_name" ), "");
        	worker_name     = StringUtil.defaultIfEmpty(rset.getString("name" ), "");
        	reg_date     = StringUtil.defaultIfEmpty(rset.getString("reg_date" ), "");
%>				
			<tr>
				<td align="center">
					<a href="javascript:goDisplay('<%=banner_id%>');" class="skin">
					<%=banner_conference_name%>
					</a>
				</td>
				<td align="center"><a href="javascript:goDisplay('<%=banner_id%>');" class="skin"><%=reg_date%></a></td>
				<td align="center"><a href="javascript:goDisplay('<%=banner_id%>');" class="skin"><%=worker_name%></a></td>
			</tr>
<%      } 
    }
    %> 
