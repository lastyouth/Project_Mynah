...
<%
    ...
    parmValue.put("ORDER_BY"            , " reg_date desc   "      );     
    ...
    // 목록
    rset = conference.getConferenceList(parmValue);

    /*============= 2. 페이징 관련 셋팅 ================*/
    totalCnt  = conference.getConferenceTotalRow(parmValue);             // 총 게시물 갯수
...
<!--
    ...
    /* 읽기 페이지로 이동 */
    function goDisplay(conference_id) {
        board.conference_id.value = conference_id; 
...
%>				
			<tr>
				<td align="center">
					<a href="javascript:goDisplay('<%=conference_id%>');" class="skin">
					<%=name%>
					</a>
				</td>
				<td align="center"><a href="javascript:goDisplay('<%=conference_id%>');" class="skin"><%=start_date%></a></td>
				<td align="center"><a href="javascript:goDisplay('<%=conference_id%>');" class="skin"><%=end_date%></a></td>
				<td align="center"><a href="javascript:goDisplay('<%=conference_id%>');" class="skin"><%=reg_date%></a></td>
			</tr>
<%      } 
    }
    %> 
      ...
      <tr>
        <td height="50" align="center" class="line_search"><select name="searchTarget">
            <option value="1" <%= "1".equals(searchTarget) ? "selected" : "" %> >컨퍼런스</option>
          </select>
