...
<%
    ...
    String map_conference_id = "";
    String conference_name = "";
    String map_id  = "";
    String map_conference_name = "";
    String title = "";
    String reg_date = "";
    ...
    // 목록
    rset = map.getMapList2(parmValue);

    /*============= 2. 페이징 관련 셋팅 ================*/
    totalCnt  = map.getMapTotalRow(parmValue);             // 총 게시물 갯수
...
<!--
    ...
    /* 읽기 페이지로 이동 */
    function goDisplay(map_conference_id) {
        board.conference_id.value = map_conference_id; 
        board.b_mode.value       = "R";
       // board.target             = "_self";
        board.action             = "<%= request.getRequestURI() %>";
        board.submit();
...
<form name="board" method="post">  
  ...
  <input type="hidden" name="map_conference_id">  
...
<%    
        ...
        while( rset.next()){
        	...
        	map_conference_name     = StringUtil.defaultIfEmpty(rset.getString("conference_name" ), "");
        	title     = StringUtil.defaultIfEmpty(rset.getString("title" ), "");
        	map_conference_id	= StringUtil.defaultIfEmpty(rset.getString("conference_id" ), "");
%>				
			<tr>
				<td align="center">
					<a href="javascript:goDisplay('<%=map_conference_id%>');" class="skin">
					<%=map_conference_name%>
					</a>
				<td align="center"><a href="javascript:goDisplay('<%=map_conference_id%>');" class="skin"><%=title%></a></td>
				</td>
			</tr>
<%      } 
    }
